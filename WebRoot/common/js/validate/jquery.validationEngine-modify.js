...
(function($) {
	...
	var methods = {
		...
		updatePromptsPosition : function() {
			...
			form.find('[class*=validate]').not(":disabled") 
					.each(
							function() {
								var field = $(this);
		...
		_onSubmitEvent : function() {
			...
			var r = methods._validateFields(form, true), _r = (!options.customValidate || options
...
//				}
//			alert(document.getElementsByName(fieldName).length)
//			alert($("input[name='" + fieldName + "']").size())
//			alert($(document.getElementsByName(fieldName)).find("input"))
//			alert($(methods._getInputWithNameAndChecked(fieldName)).length);
			...
			if (fieldType == "text"&& fieldName && inputFields.size() > 1) {
				...
				//		+ "']:last"));
				//上面是一个bug,如果同一个页面内，有两个标签name相同，但不再同一个form内，就会出现问题
				//修改为一下
		...
		_required : function(field, rules, i, options) {
			...
			case "checkbox":
				...
				if(!name){
					if(field.is("input:checkbox")&&!field.is("input:checked")){
						return options.allrules[rules[i]].alertTextCheckboxMultiple;
					}
					break;
				}
				if ($(methods._getInputWithNameAndChecked(name)).size() == 0) {
...
//				// added by paul@kinetek.net for select boxes, Thank you
//				alert(!field.find("option:selected").val())
...
//				break;
		...
		_buildPrompt : function(field, promptText, type, ajaxed, options) {
			...
			//在提示内容前加入错误小图标
//			$('<i>').prependTo(promptContent);
			
			//加入左侧小指示箭头		
			var tooltipfang = $('<div>').addClass("tooltipfang").appendTo(prompt);	
...
//			alert(field.attr('id'))
			...
			if (options.isOverflown){
				...
				//field.parents('form').append(prompt);
				//field.parents('form').append(prompt);
