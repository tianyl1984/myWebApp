...
public class JavaAgentUtil {
	...
	private static void createJar2() {
		String zipFile = "C:/Users/tianyl/git/mywebapp/src/main/java/com/hzth/myapp/javaAgent/agent2.jar";
		...
		try {
			...
			mf.append("Agent-Class: com.hzth.myapp.javaAgent.AgentmainDemo\r\n");
			...
			for (File f : file.listFiles()) {
				if (f.getName().startsWith("AgentmainDemo")) {
	...
	private static void createJar() {
		...
		try {
			...
			File file = new File(path).getParentFile();
			for (File f : file.listFiles()) {
				if (f.getName().startsWith("PremainAgentDemo") || f.getName().startsWith("MonitorTransformer")) {
					String name = f.getPath().replaceFirst(".*com\\\\hzth\\\\myapp", "com\\\\hzth\\\\myapp");
					name = name.replaceAll("\\\\", "/");
					out.putNextEntry(new ZipEntry(name));
					FileInputStream fis = new FileInputStream(f);
